...
//___________________________ Controlador ___________________________ 
			...
			if(ADC1ConvertedVoltage > 1900 && ADC1ConvertedVoltage < 2900){
				...
				if(error_actual > -0.05	&& error_actual < 0.05){ //Si el error es muy pequeño la corriente se fija igual al set point esto por las variaciones en medicion que tiene el ADC
					corriente = setPoint; 
				}else{
...
void ADCConfig(void){
	...
	Delay(10); // Insert delay equal to 10 �s
